...
public class GlobalExceptionHandler {
    ...
    public ProblemDetail handleTransactionNotFound(TransactionNotFoundException ex) {
        ...
        problem.setType(URI.create(PROBLEM_BASE_URI + "transaction-not-found"));
    ...
    public ProblemDetail handleInvoiceNotFound(InvoiceNotFoundException ex) {
        ...
        ProblemDetail problem = ProblemDetail.forStatusAndDetail(HttpStatus.NOT_FOUND, ex.getMessage());
        problem.setType(URI.create(PROBLEM_BASE_URI + "invoice-not-found"));
    ...
    public ProblemDetail handleInvalidPaymentOperation(InvalidPaymentOperationException ex) {
        log.warn("Invalid payment operation: {}", ex.getMessage());
        ProblemDetail problem = ProblemDetail.forStatusAndDetail(HttpStatus.UNPROCESSABLE_ENTITY, ex.getMessage());
        problem.setType(URI.create(PROBLEM_BASE_URI + "invalid-payment-operation"));
        problem.setTitle("Invalid Payment Operation");
        problem.setProperty("timestamp", Instant.now());
        return problem;
    }
    ...
    @ExceptionHandler(ContractServiceUnavailableException.class)
    public ProblemDetail handleContractServiceUnavailable(ContractServiceUnavailableException ex) {
        log.error("Contract Service unavailable: {}", ex.getMessage());
        ProblemDetail problem = ProblemDetail.forStatusAndDetail(HttpStatus.SERVICE_UNAVAILABLE, ex.getMessage());
        problem.setType(URI.create(PROBLEM_BASE_URI + "contract-service-unavailable"));
        problem.setTitle("Contract Service Unavailable");
        problem.setProperty("timestamp", Instant.now());
        return problem;
    }
