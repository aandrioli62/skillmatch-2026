...
| `user.validated` | user-service | notification-service | `notification.all-events` | Un amministratore ha validato un utente, professionista o azienda (`POST /api/v1/admin/users/{userId}/validate`); l'utente viene notificato e può ora candidarsi ai progetti (professionista) o crearne di nuovi (azienda). |
...
| `candidature.submitted` | project-service | notification-service | `notification.all-events` | Un professionista si è candidato a un progetto (`POST /api/v1/projects/{id}/candidatures`). Il Notification Service avvisa l'azienda proprietaria del progetto. |
| `candidature.accepted` | project-service | contract-service, notification-service | `contract.candidature.accepted`, `notification.all-events` | L'azienda ha accettato una candidatura (`PUT /api/v1/projects/{id}/candidatures/{candidatureId}/accept`). Il Contract Service crea automaticamente un micro-contratto in stato `DRAFT` con l'importo del progetto e la commissione corrente; il Notification Service avvisa sia il professionista selezionato sia l'azienda. L'accettazione rifiuta automaticamente anche tutte le altre candidature ancora `PENDING` per lo stesso progetto e porta il progetto a `ASSIGNED` (l'azienda può anche rifiutare una singola candidatura senza accettarne un'altra, con `PUT /api/v1/projects/{id}/candidatures/{candidatureId}/reject`, che non pubblica alcun evento). |
...
- **Email reale, non solo notifica in-app**: per un sottoinsieme deliberatamente ristretto di eventi (`user.validated`, `candidature.accepted`, `payment.completed`), oltre a salvare la notifica in MongoDB il Notification Service invia anche una vera email transazionale tramite `MailService` (SMTP relay Brevo), per non riempire le caselle di posta a ogni evento e non esaurire la quota gratuita del provider. Per risolvere l'indirizzo email del destinatario, il listener RabbitMQ (che non ha un JWT dell'utente da inoltrare, a differenza delle chiamate REST in arrivo dal gateway) autentica la chiamata a `GET /api/v1/users/{id}` sullo User Service con un token OAuth2 **Client Credentials**, ottenuto dal client Keycloak confidenziale `notification-service` (`serviceAccountsEnabled: true`). È il primo e unico uso concreto nel sistema del flusso Client Credentials descritto in [ADR-002](adr/ADR-002-keycloak-configuration.md): tutte le altre chiamate REST sincrone service-to-service inoltrano invece il JWT dell'utente chiamante. Un fallimento dell'invio email viene solo loggato, mai propagato: un relay SMTP irraggiungibile non deve impedire la persistenza della notifica in-app.
